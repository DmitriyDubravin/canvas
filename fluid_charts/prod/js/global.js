...
var zeros = Array.apply(null, Array(5)).map(function () {
	return appHeight + 20;
});
var arr1 = [100, 150, 220, 180, 270];
var arr2 = [300, 280, 200, 180, 120];
var arr3 = [200, 380, 300, 280, 320];
...
var Chart = function () {
	function Chart(name, coords) {
		...
		this.dots = this.coords.map(function (item) {
			return appHeight - item;
		});
		this.startDots = zeros;
		this.currentDots = this.startDots;
		this.finishDots = this.dots;
		this.movingDirs = Array.apply(null, Array(this.coords.length)).map(function () {
			return 'stop';
		});
		this.movingSteps = Array.apply(null, Array(this.coords.length)).map(function () {
			return 0;
		});
	}
	...
	_createClass(Chart, [{
		...
		value: function drawLine() {
			for (var i = 0; i < this.currentDots.length; i++) {
				...
				var startX = this.step + this.step * i;
				var middleX = startX + this.step / 2;
				var endX = startX + this.step;
				var startY = this.currentDots[i];
				var endY = this.currentDots[i + 1];
				...
				if (endY !== undefined) {
					...
					// shadows
					var totalLines = this.step * 2;
					for (var z = 0; z < totalLines; z++) {
						var y = z / totalLines;
						var points = bezierCubicXY({ x: startX, y: startY }, { x: middleX, y: startY }, { x: middleX, y: endY }, { x: endX, y: endY }, y);
						var gradient = ctx.createLinearGradient(points.x, points.y, points.x, points.y + this.shadowHeight);
		...
		key: 'calcMovingSteps',
		value: function calcMovingSteps() {
			var _this = this;

			this.movingSteps = this.finishDots.map(function (dot, i) {
				if (_this.startDots[i] > dot) {
					return (_this.startDots[i] - dot) / 10;
				} else if (_this.startDots[i] < dot) {
					return (dot - _this.startDots[i]) / 10;
				} else {
					return 0;
				}
			});
		...
		key: 'calcMovingDirections',
		value: function calcMovingDirections() {
			var _this2 = this;

			this.movingDirs = this.finishDots.map(function (dot, i) {
				if (_this2.startDots[i] > dot) {
					return 'up';
				} else if (_this2.startDots[i] < dot) {
					return 'down';
				} else {
					return 'stop';
				}
		...
		key: 'calcNextMovingDots',
		value: function calcNextMovingDots() {
			var _this3 = this;

			this.currentDots = this.currentDots.map(function (dot, i) {
				if (_this3.movingDirs[i] === 'up') {
					if (dot - _this3.movingSteps[i] > _this3.finishDots[i]) {
						return dot - _this3.movingSteps[i];
					} else {
						return _this3.finishDots[i];
					}
				} else if (_this3.movingDirs[i] === 'down') {
					if (dot + _this3.movingSteps[i] < _this3.finishDots[i]) {
						return dot + _this3.movingSteps[i];
					} else {
						return _this3.finishDots[i];
		...
		key: 'checkMovingDirections',
		value: function checkMovingDirections(currentDots, movingDirs, endDots) {
			var _this4 = this;

			this.movingDirs = this.movingDirs.map(function (dir, i) {
				if (dir === 'up' && _this4.currentDots[i] <= _this4.finishDots[i] || dir === 'down' && _this4.currentDots[i] >= _this4.finishDots[i]) {
					return 'stop';
				} else {
					return dir;
				}
...
var nextActiveChart = null;
var activeChartDots = null;
...
function show(index) {
	...
	// one -> one
	if (activeChart !== 'all' && index !== undefined) {
		charts = allCharts.filter(function (item, i) {
			return i === index;
		});
...
function move(charts) {
	...
	charts.forEach(function (chart) {
		...
		stopped.push(chart.movingDirs.some(function (item) {
			return item !== 'stop';
		}));
	});

	if (stopped.some(function (item) {
		return item;
	})) {
...
},{"core-js/fn/regexp/escape":3,"core-js/shim":328,"regenerator-runtime/runtime":329}],3:[function(require,module,exports){
'use strict';
